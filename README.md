...
2. 로그인 이후 로그아웃 방법이 없다.
3. 공지사항 폼으로 돌아갈 방법이 없다.
4. 발주관리
 -콤보박스 선택 후 좌측 텍스트박스에 올바르지 않은 데이터 형식을 검색하면 UI가 멈추는 현상이 발생한다.
 -수정 시 입력 데이터 형식이 올바르지 않으면 UI가 멈추는 현상이 발생한다.
 -추가 시 입력 데이터 형식이 올바르지 않으면 UI가 멈추는 현상이 발생한다.
5. 창고관리
 -조회 부분에 알맞은 DB테이블을 못 불러온다.
 -수정 시 입력 데이터 형식이 올바르지 않으면 UI가 멈추는 현상이 발생한다.
 -추가 시 입력 데이터 형식이 올바르지 않으면 UI가 멈추는 현상이 발생한다.
 -수정창과 추가창이 메인 화면 센터에 표시되지 않는다.
